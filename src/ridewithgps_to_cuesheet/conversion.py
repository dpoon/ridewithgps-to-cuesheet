...
def _read_as_cue(row: List[str], idx: int, last_dist: Decimal, opts: GenerationOptions) -> Cue:
    has_end = False
    is_control = row[0] in opts.control_cue_indicators or re.match(r"^Control.*?:", row[1])
...
def _map_cue_description(opts: GenerationOptions, description: str) -> str:
    ...
    elif match := re.match("Control.*?: *(?<control_name>.*)", description):
        return match.group('control_name')
    elif match := re.match(r"^At roundabout, take exit (?<exit>\d+) [io]nto (?<road>.*)", description):
        return f"{match.group('road')} (roundabout exit {match.group('exit')})"
    elif match := re.match(r"^Continue (?:straight )?[io]nto (?<road>.*)", description):
        return match.group('road')
    elif match := re.match(r"^(?:Keep|Turn) (?:slight )?(?:left|right) [io]nto (?<road>.*)", description):
        return match.group('road')
    elif match := re.match(r"^(?:Make a )?U-turn on(?:to)? (?<road>.*)", description):
        return match.group('road')
    elif match := re.match(f"Turn (?<direction>left|right) to ([^(stay)])", description):

...
def _create_excel_formats(workbook):
    ...
    return {
        "title_format": workbook.add_format(merge_dicts({"rotation": 90}, defaults, all_border)),
        "descr_format": workbook.add_format(merge_dicts(centered, defaults, all_border)),
        ...
        "arial_12": workbook.add_format(merge_dicts(a_12_opts, all_border)),
        "arial_12_no_border": workbook.add_format(
            merge_dicts(a_12_opts, all_border, {"left_color": "white", "right_color": "white"})
        ),
        "dist_format": workbook.add_format(merge_dicts({"num_format": "0.00"}, float_top, a_12_opts, all_border)),
        "dist_format2": workbook.add_format(merge_dicts({"num_format": "0.0"}, float_top, a_12_opts, all_border)),
        "cue_format": workbook.add_format(merge_dicts({"text_wrap": True}, float_top, a_12_opts, all_border)),
        "red_title": workbook.add_format(merge_dicts({"font_color": "red"}, a_12_opts, centered)),
        "black_title": workbook.add_format(merge_dicts({"font_color": "black"}, a_12_opts, centered)),
    }
...
def _setup_worksheet_headers(worksheet, formats, opts, values_array):
    ...
    worksheet.write(as_letter(curr_col) + "6", "Route Description", formats["descr_format"])

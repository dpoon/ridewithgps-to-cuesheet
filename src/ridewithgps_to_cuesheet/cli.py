...
def download_route(url_info: dict, verbose: bool = False) -> None:
...
def organize_output_files(excel_filename: str, csv_filename: Optional[str] = None, is_url: bool = False) -> None:
    """Move generated files to appropriate directories."""
    try:
        # Move Excel file to outputs directory
        output_path = Path(XLSX_DIR) / excel_filename
        ...
        # Handle CSV file cleanup/organization
        if is_url and Path(TMP_CURL_FILE).exists():
            Path(TMP_CURL_FILE).unlink()  # Remove temporary file
        elif csv_filename and not csv_filename.startswith(CSV_DIR):
            csv_path = Path(CSV_DIR) / Path(csv_filename).name
